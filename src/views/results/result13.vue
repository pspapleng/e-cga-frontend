<template>
  <div class="hero is-fullheight">
    <div class="hero-body py-3">
      <div class="container">
        <div class="columns">
          <!-- <div class="column is-1">
            <Sidebar />
          </div> -->
          <div
            class="column is-11-desktop is-9-tablet is-7-mobile is-offset-1-desktop is-offset-2-tablet is-offset-3-mobile"
          >
            <div class="card">
              <header class="card-header">
                <p
                  class="card-header-title"
                  style="color: white; background-color: #1E3A8A"
                >
                  แบบประเมินความเสี่ยงต่อโรคกระดูกพรุน
                </p>
              </header>
              <div class="card-content">
                <div class="content has-text-left">
                  <p class="title">ข้อมูลผู้เข้ารับบริการ</p>
                  <p class="ml-4">
                    น้ำหนัก
                    <mark style="background-color: #E0E0E0;">
                      <span class="mx-5">{{ user.weight }}</span>
                    </mark>
                    กิโลกรัม
                  </p>
                  <p class="ml-4">
                    อายุ
                    <mark style="background-color: #E0E0E0;">
                      <span class="mx-5">{{ age }}</span>
                    </mark>
                    ปี
                  </p>
                </div>
              </div>
            </div>
            <div
              class="card"
              v-for="ques in form.slice(162, 163)"
              :key="ques.ques_id"
            >
              <div class="card-content">
                <div class="content has-text-left">
                  <p class="title">การคำนวณค่า OSTA</p>
                  <p class="ml-4">
                    สูตร
                    <mark style="background-color: #E0E0E0;">
                      <span class="mx-5"> 0.2 x (น้ำหนักตัว - อายุ)</span>
                    </mark>
                  </p>
                  <p class="ml-4">
                    แทนค่า 0.2 x (
                    <mark style="background-color: #E0E0E0;">
                      <span class="mx-5">{{ user.weight }}</span>
                    </mark>
                    -
                    <mark style="background-color: #E0E0E0;">
                      <span class="mx-5">{{ age }}</span>
                    </mark>
                    )
                  </p>
                  <div v-if="ans[ques.ques_id - 1].ans_value === -1">
                    ค่าที่คำนวณได้เท่ากับ
                    <mark style="background-color: #1E3A8A; color: white">
                      <span class="mx-2">{{
                        ans[ques.ques_id - 1].ans_title
                      }}</span>
                    </mark>
                  </div>
                </div>
              </div>
            </div>
            <div class="card">
              <div class="card-content">
                <div class="content has-text-centered">
                  <p class="title">ตารางประเมินความเสี่ยง</p>
                  <img style="width: 50%" src="@/assets/ass_table1.png" />
                </div>
              </div>
            </div>

            <div class="columns mt-4">
              <div class="column is-2">
                <router-link to="/results/result12">
                  <b-button class="mr-2">
                    <b-icon icon="chevron-left"> </b-icon>
                  </b-button>
                </router-link>
                <b-button disabled>
                  <b-icon icon="chevron-right"> </b-icon>
                </b-button>
              </div>
              <div class="column is-8 is-offset-4">
                <router-link to="/results">
                  <b-button
                    class="back mr-2"
                    type="is-light"
                    style="font-family: 'Kanit', sans-serif; font-weight: 400; color: #1E3A8A"
                    >กลับสู่หน้าหลัก</b-button
                  >
                </router-link>
                <b-button
                  class="assess"
                  type="is-light"
                  @click="isAssess = true"
                  style="font-family: 'Kanit', sans-serif; font-weight: 400; color: #047857"
                  >ประเมินผล</b-button
                >
              </div>
            </div>
          </div>
          <div class="column is-1-desktop is-3-tablet is-5-mobile">
            <Sidebar />
          </div>
        </div>

        <!-- ผลประเมิน -->
        <b-modal v-model="isAssess" :width="640">
          <div class="card">
            <header class="card-header">
              <p
                class="card-header-title"
                style="color: white; background-color: #1E3A8A"
              >
                ผลการประเมินความเสี่ยงต่อโรคกระดูกพรุน
              </p>
            </header>
            <div class="card-content" style="background-color: #f4f4f4">
              <div class="content has-text-left ml-6">
                การพิจารณา
                <br />
                ค่า OSTA น้อยกว่า -4 = ความเสี่ยงสูง
                <br />
                ค่า OSTA ระหว่าง -4 ถึง -1 = ความเสี่ยงปานกลาง
                <br />
                ค่า OSTA ระหว่างมากกว่า -1 = ความเสี่ยงต่ำ
              </div>
              <div class="card">
                <div class="card-content has-text-centered">
                  <div class="content">
                    <p class="title mx-5">
                      {{ user.result.OSTA }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <footer class="card-footer">
              <p
                class="card-footer-item"
                @click="isAssess = false"
                style="color: #F90000"
              >
                ย้อนกลับ
              </p>
              <router-link class="card-footer-item" to="/results">
                <p style="color: #047857">
                  เสร็จสิ้นการดูผลประเมิน
                </p>
              </router-link>
            </footer>
          </div>
        </b-modal>
      </div>
    </div>
  </div>
</template>
<script>
import Sidebar from "@/components/sidebar.vue";
import { mapState, mapActions } from "vuex";

export default {
  components: {
    Sidebar
  },
  name: "result13",
  data() {
    return {
      isAssess: false
    };
  },
  computed: {
    ...mapState({
      form: "json",
      ans: "ans",
      user: "owner"
    }),
    age() {
      var today = new Date();
      var birthDate = new Date(this.user.date_of_birth);
      var age = today.getFullYear() - birthDate.getFullYear();
      var m = today.getMonth() - birthDate.getMonth();
      if (m < 0 || (m === 0 && today.getDate() < birthDate.getDate())) {
        age--;
      }
      return age;
    }
  },
  methods: {
    ...mapActions(["getAns"])
  },
  beforeRouteEnter(to, from, next) {
    console.log("before");
    next(vm => {
      vm.getAns();
    });
  }
};
</script>
<style>
.card-header-title {
  font-size: 1.5rem;
  font-weight: 500;
}
.head {
  font-size: 1.125rem;
  font-weight: 500;
  margin-bottom: 0.75rem;
}
.title {
  font-size: 1rem;
  font-weight: 500;
  margin-bottom: 0.75rem;
}
.card {
  margin-top: 3vh;
}
</style>
